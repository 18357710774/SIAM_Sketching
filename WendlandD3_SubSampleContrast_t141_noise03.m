...
k_rbf = 3;
delta = 1; 
xc = eq_point_set(d-1,nc);  
x = SD(L);
N = size(x, 1);
y0 = rbf_multicentre(x, k_rbf, delta, xc');

% point for evaluation
Nev = 10000;
[~, xev] = SP(Nev);
yev = rbf_multicentre(xev, k_rbf, delta, xc');

% compute the kernel matrix
KerPara.KernelType = 5; 
KerPara.para = [k_rbf; delta];
...
for t = t_seq
    ...
    for Ex = 1:ExNum
       ...
       %% select the first m samples
        ...
        RMSE_tr_subsetfir(Ex, kk) = sqrt(mean((y0 - hat_y_subsetfirtmp).^2));
       ...
       %% randomly select m samples
        ...
        RMSE_tr_subsetrand(Ex, kk) = sqrt(mean((y0 - hat_y_subsetrandtmp).^2));
        ...
        RMSE_tr_subtdesigns(Ex, kk) = sqrt(mean((y0 - hat_y_subtdesignstmp).^2));
...
save([p1 '\SynResults\Wendland_N' num2str(N) 'D' num2str(d) ...
    '_SubSampleContrast_t' num2str(L) '_noise' num2str(noise) '.mat'],...
    'ExNum', 'KerPara', 'N', 'L', 'd', 'nc', 'noise', 'k_rbf', 'delta', 'lambda', 'y_cell',...
     't_seq', 'xc', 'x', 'y0', 'xev', 'yev', 'N_subset_vec', ...
